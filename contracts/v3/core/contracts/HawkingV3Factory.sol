...
/// @title Canonical HawkingSwap V3 factory
/// @notice Deploys HawkingSwap V3 pools and manages ownership and control over pool protocol fees
contract HawkingV3Factory is IHawkingV3Factory {
    /// @inheritdoc IHawkingV3Factory
    ...
    function createPool(
        ...
        pool = IHawkingV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IHawkingV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IHawkingV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IHawkingV3Pool(pool).setLmPool(lmPool);
    }
}
